...
def top_k_span_endpoints(start_probs, end_probs, window=15, k=5):
    """
    based on the search_span_endpoints
    Returns:
        Top k optimal pairs of starting and ending indices for the answer span. 
    """
    top_k_anws = []
    max_start_index = start_probs.index(max(start_probs))
    max_joint_prob = 0.
    for end_index in range(len(end_probs)):
        if max_start_index <= end_index <= max_start_index + window:
            joint_prob = start_probs[max_start_index] * end_probs[end_index]
            if len(top_k_anws) < k:
                heapq.heappush(top_k_anws, (joint_prob, max_start_index, end_index))
            else:
                heap_min, min_start, min_end = heapq.heappop(top_k_anws)
                if joint_prob > heap_min:
                    heapq.heappush(top_k_anws, (joint_prob, max_start_index, end_index))
                else:
                    heapq.heappush(top_k_anws, (heap_min, min_start, min_end))

    top_k_anws.sort(reverse=True)
    return top_k_anws
