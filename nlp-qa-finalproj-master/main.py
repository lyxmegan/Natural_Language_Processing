...
from utils import cuda, search_span_endpoints, unpack, top_k_span_endpoints
...
def apply_multiplier(k, lower=1.0, upper=1.5):
    multipliers = []
    current_multiplier = lower
    added = (upper - lower) / k
    
    while current_multiplier < upper:
        current_multiplier += added
...
def new_probs_based_on_ner(top_k_answers, passage_truecase, find_interrogative, k, lower, upper):
    if (find_interrogative != 'who' and find_interrogative != 'where' and find_interrogative != "when"):
        return top_k_answers
    
    answers = []
    for max_prob, start_index, end_index in top_k_answers:
        pred_span = str(passage_truecase[start_index:(end_index + 1)])
        ans_ents = nlp(pred_span).ents
        mapped_ents = ner_dict[find_interrogative]
        count_matching_ents = 0
        for ent in ans_ents:
            if ent.label_ in mapped_ents:
                count_matching_ents += 1
           
        heapq.heappush(answers, (-count_matching_ents, max_prob, start_index, end_index))

    get_multipliers = apply_multiplier(k, lower, upper)
    new_preds = []
    index = 0
    while answers:
        temp = heapq.heappop(answers)
        temp = ((temp[1] * get_multipliers[index]), temp[2], temp[3])
        new_preds.append(temp)
        index += 1
    new_preds.sort(reverse=True)
    return new_preds

def write_predictions(args, model, dataset, dataset_truecase, gold_answers):
    """
    ...
    with torch.no_grad():
        for (i, batch) in enumerate(test_dataloader):
            ...
            for j in range(start_logits.size(0)):
                ...
                get_question = question_truecase
                
                question_ents = set(nlp(" ".join(get_question)).ents)
                question_ents_text = set([ent.text for ent in question_ents])
                
                #Finds the first interrogative in the question 
                first_interrogative = ''
                question_words = [w.lower() for w in get_question]
                ...
                topk = top_k_span_endpoints(start_probs, end_probs)
                
                prev_topk = topk
                lower = 1.0
                upper = 1.5
                topk = new_probs_based_on_ner(topk, passage_truecase, first_interrogative, 5, lower, upper)
               
                temp = topk[0] if topk else prev_topk[0]
                final_start, final_end = temp[1], temp[2]

                # get predicted span.
                pred_span = ' '.join(passage[final_start:(final_end + 1)])

                prev_best_answer = prev_topk[0]
                prev_start, prev_end = prev_best_answer[1], prev_best_answer[2]
                prev_pred_span = ' '.join(passage[prev_start:(prev_end + 1)])

                pred_span = pred_span.replace(" - ", "-")
                prev_pred_span = prev_pred_span.replace(" - ", "-")
...
def main(args):
    ...
    if args.do_test:
        ...
        gold_answers = read_answers(args.dev_path)
        write_predictions(args, model, dev_dataset, dev_dataset_truecase, gold_answers)
        eval_cmd = (
            'python3 evaluate.py '
