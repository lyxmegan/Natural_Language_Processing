...
class Seq2SeqSemanticParser(nn.Module):
    def __init__(self, input_indexer, output_indexer, emb_dim, hidden_size, input_max_len, embedding_dropout=0.2, bidirect=True):
        ...
        self.decoder = AttentionDecoder(emb_dim, hidden_size, len(output_indexer), input_max_len)
    ...
    def decode(self, test_data: List[Example]) -> List[List[Derivation]]:
        ...
        input_data = make_padded_input_tensor(test_data, self.input_indexer, input_max_len)
        idx_sos = torch.tensor(self.output_indexer.index_of(SOS_SYMBOL)).unsqueeze(0)
        idx_eos = torch.tensor(self.output_indexer.index_of(EOS_SYMBOL)).unsqueeze(0) 
        sos_embed_input = self.output_emb.forward(idx_sos).unsqueeze(0)
        
        predictions= []
        for idx in range(len(test_data)):
            y_toks = []
            x_tensor = torch.from_numpy(input_data[idx]).unsqueeze(0)
            inp_lens_tensor = torch.from_numpy((np.array(len(test_data[idx].x_tok)))).unsqueeze(0)
            enc_output_each_word, _, enc_final_states = self.encode_input(x_tensor, inp_lens_tensor)
            preds, dec_hid = self.decoder.forward(sos_embed_input, torch.tensor(1).unsqueeze(0), enc_final_states, enc_output_each_word)
            dec_hid = (dec_hid[0].unsqueeze(0),dec_hid[1].unsqueeze(0))
            output_idx = torch.max(preds, dim = 1)[1]
            count = 0
            while output_idx != idx_eos and count <= 65:
                count += 1
                y_toks.append(self.output_indexer.get_object(output_idx.item()))
                out_emb = self.output_emb.forward(output_idx).unsqueeze(0)
                preds, dec_hid = self.decoder.forward(out_emb, torch.tensor(1).unsqueeze(0), dec_hid, enc_output_each_word)
                dec_hid = (dec_hid[0].unsqueeze(0),dec_hid[1].unsqueeze(0))
                output_idx = torch.max(preds, dim = 1)[1]
            predictions.append([Derivation(test_data[idx], 1.0, y_toks)])
...
class AttentionDecoder(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, max_input_len):
        ...
        #add attention
        self.alignment = nn.Linear(hidden_size, hidden_size*2)
        self.W = nn.Linear(hidden_size *3, output_size, bias=True)
        self.init_weight()
    ...
    def forward(self, input_emb, input_lens, hidden_input, enc_output_states):
        packed_embedding = nn.utils.rnn.pack_padded_sequence(input_emb, input_lens, batch_first=True)
        ...
        #modify to attention
        #dot product
        e_ij = torch.mm(self.alignment(h),enc_output_states.squeeze(1).t()) 
        #caculate alpha
        alpha_ij = torch.softmax(e_ij,dim=1)
        #weighted sum of input hidden states 
        c_i = torch.mm(alpha_ij,enc_output_states.squeeze(1))
        c_i_final = torch.tanh(torch.cat((c_i,h), dim=1))
        preds = self.W(c_i_final)
        return (preds, h_t)
...
def train_model_encdec(train_data: List[Example], dev_data: List[Example], input_indexer, output_indexer, args) -> Seq2SeqSemanticParser:
    ...
    emb_dim = 100
    hidden_size = 200
    model = Seq2SeqSemanticParser(input_indexer, output_indexer, emb_dim, hidden_size, input_max_len)
    ...
    #optimizer
    params = list(model.decoder.parameters()) + list(model.encoder.parameters()) + list(model.input_emb.parameters()) + list(model.output_emb.parameters())
    ...
    for epoch in range(epochs):
        ...
        for idx in ex_indices:
            ...
            model.input_emb.zero_grad()
            model.output_emb.zero_grad()
            ...
            enc_output_each_word, _, enc_final_states = model.encode_input(x_tensor, inp_lens_tensor)

            SOS_embed = model.output_emb.forward(SOS_idx).unsqueeze(0)
            preds, dec_hid = model.decoder.forward(SOS_embed, torch.tensor(1).unsqueeze(0), enc_final_states, enc_output_each_word)
            
            y_indices = train_data[idx].y_indexed
            for gold_idx in y_indices:
                loss += criterion(preds, torch.LongTensor([gold_idx]))

                if gold_idx == EOS_idx:
                    break
                dec_hid = (dec_hid[0].unsqueeze(0),dec_hid[1].unsqueeze(0))
                input_emb = model.output_emb.forward(torch.tensor(gold_idx).unsqueeze(0)).unsqueeze(0)
                preds, dec_hid = model.decoder.forward(input_emb, torch.tensor(1).unsqueeze(0), dec_hid, enc_output_each_word)
            
            loss.backward()                
            optimizer.step()
            total_loss += loss.item()
        print("Total loss on epoch %i: %f" % (epoch, total_loss))
    print("Total training time:", time.time()-time_s)
    return model