...
class HmmNerModel(object):
    ...
    def decode(self, sentence_tokens: List[Token]) -> LabeledSentence:
        ...
        """
        prob_seq_scorer = ProbabilisticSequenceScorer(self.tag_indexer, self.word_indexer, self.init_log_probs,
                                             self.transition_log_probs, self.emission_log_probs)
        
        token_counts = len(sentence_tokens)
        tag_counts = len(self.tag_indexer)
        predict_tags = []

        # initialize viterbi and backpointer
        viterbi = np.zeros((token_counts, tag_counts))
        back_pointer = np.zeros((token_counts, tag_counts))
        
        #bt[0,:] already set to 0's
        for i in range(tag_counts):
            viterbi[0, i] = self.init_log_probs[i] + prob_seq_scorer.score_emission(sentence_tokens, i, 0)
        
        # recursion
        for t in range(1, token_counts):
            for j in range(tag_counts):
                max_values = np.zeros(tag_counts)
                for i in range(tag_counts):
                    max_values[i] = viterbi[t - 1, i] + self.transition_log_probs[i, j] + prob_seq_scorer.score_emission(sentence_tokens, j, t)
                back_pointer[t, j] = np.argmax(max_values)
                viterbi[t, j] = np.max(max_values)
        ...
        for i in range(len(sentence_tokens) - 1, 0, -1):
            predict_tags.append(self.tag_indexer.get_object(np.int(back_pointer[i, tag_index])))
            tag_index = np.int(back_pointer[i, tag_index])

        predict_tags.reverse()
        chunks = chunks_from_bio_tag_seq(predict_tags)
...
class CrfNerModel(object):
    ...
    def decode(self, sentence_tokens: List[Token]) -> LabeledSentence:
        ...
        fbs_scorer = FeatureBasedSequenceScorer(self.tag_indexer, self.feature_weights, feature_cache)
        viterbi = np.zeros((num_words,num_tags))
        back_pointer = np.zeros((num_words,num_tags))
        #Inititalization
        for tag_idx in range(num_tags):
            viterbi[0][tag_idx] = fbs_scorer.score_emission(sentence_tokens, tag_idx, 0)
            
        #Recursion
        for word_idx in range(1,num_words):
            for tag_idx in range(num_tags):
                s_em = fbs_scorer.score_emission(sentence_tokens, tag_idx, word_idx)
                for prev_tag_idx in range(num_tags):
                    s_trans = fbs_scorer.score_transition(sentence_tokens, prev_tag_idx, tag_idx)
                    cur_score = viterbi[word_idx-1][prev_tag_idx] + s_trans + s_em
                    if prev_tag_idx == 0:
                        viterbi[word_idx][tag_idx] = cur_score
                        back_pointer[word_idx][tag_idx] = prev_tag_idx
                    elif cur_score > viterbi[word_idx][tag_idx]:
                        viterbi[word_idx][tag_idx] = cur_score 
                        back_pointer[word_idx][tag_idx] = prev_tag_idx
                
        #Termination step
        max_score = viterbi[num_words-1][0]
        bp = 0
        for tag_idx in range(num_tags):
            if viterbi[num_words-1][tag_idx] > max_score:
                max_score = viterbi[num_words-1][tag_idx]
                bp = tag_idx
    ...
    def decode_beam(self, sentence_tokens: List[Token]) -> LabeledSentence:
        ...
        for tag_idx in range(num_tags):
            viterbi[0][tag_idx] = fbs_scorer.score_emission(sentence_tokens, tag_idx, 0)
            beam.add(self.tag_indexer.get_object(tag_idx),viterbi[0][tag_idx])
        beam_list.append(beam)
            
        #Recursion
        for word_idx in range(1,num_words):
            beam = Beam(2)
            for tag_idx in beam_list[word_idx-1].get_elts_and_scores():
                #s_em = fbs_scorer.score_emission(sentence_tokens, tag_idx, word_idx)
                j = self.tag_indexer.index_of(tag_idx[0])
                s_em = fbs_scorer.score_emission(sentence_tokens, j, word_idx)
                for s in range(num_tags):
                    s_trans = fbs_scorer.score_transition(sentence_tokens, s, j)
                    cur_score = viterbi[word_idx-1][s] + s_trans + s_em
                    if s == 0:
                        viterbi[word_idx][j] = cur_score
                        #back_pointer[word_idx][j] = prev_tag_idx
                        beam.add(self.tag_indexer.get_object(s), cur_score)
                    elif cur_score > viterbi[word_idx][j]:
                        viterbi[word_idx][tag_idx] = cur_score 
                        #back_pointer[word_idx][tag_idx] = prev_tag_idx
                        beam.add(self.tag_indexer.get_object(s), cur_score)
            beam_list.append(beam)
...
def compute_gradient(sentence: LabeledSentence, tag_indexer: Indexer, scorer: FeatureBasedSequenceScorer, feature_indexer: Indexer) -> (float, Counter):
    ...
    for tag_idx in range(num_tags): 
        alpha[0][tag_idx] = scorer.score_emission(sentence.tokens, tag_idx, 0) # Emission log probability
        beta[num_words - 1][tag_idx] = 0       # log(1) = 0

    """ forward_score_emission = []
    forward_score_trans = []
    for word_idx in range(1, num_words):
        for cur_tag_idx in range(num_tags):
            emission_log_prob = scorer.score_emission(sentence.tokens, cur_tag_idx, word_idx)
            forward_score_emission.append(emission_log_prob) 
            for prev_tag_idx in range(num_tags):
                transition_log_prob = scorer.score_transition(sentence.tokens, prev_tag_idx, cur_tag_idx)
                backward_score_trans.append(transition_log_prob)
            """
    ...
    """ backward_score_emission = []
    backward_score_trans = []
    for word_idx in reversed(range(num_words-1)):
        for cur_tag_idx in range(num_tags):
            for next_tag_idx in range(num_tags):
                emission_log_prob = scorer.score_emission(sentence.tokens, next_tag_idx, word_idx+1)
                backward_score_emission.append(emission_log_prob )
                transition_log_prob = scorer.score_transition(sentence.tokens, cur_tag_idx, next_tag_idx)
                backward_score_trans.append(transition_log_prob)
    ...
    for word_idx in range(num_words):
        ...
        for feature_pos in scorer.feat_cache[word_idx][gold_tag_idx]:
            gradient_counter[feature_pos] += 1
        
        for tag_idx in range(num_tags):
            for feature_pos in scorer.feat_cache[word_idx][tag_idx]:
                gradient_counter[feature_pos] -= np.exp(log_marg_probs[word_idx][tag_idx])

    return 0.0, gradient_counter
